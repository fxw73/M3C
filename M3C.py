...
class CIFAR10_Classifier(BaseModelDNN):
    ...
    def _fit_one_epoch(self, train_loader, epoch, logger, is_AdvTr=False, adversary=None, device=torch.device("cuda")):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            target_softmax_out_temp = nn.Softmax(dim=1)(logits_ce)
            ...
            loss_ACL = torch.mean(
                target_entropy_weight * target_softmax_out_temp[:, 0] *
                target_softmax_out_temp[:, 1])

            loss = self.criterion((logits_ce, logits_CAS), target) + 4.0 * loss_ACL
            loss.backward()
            self.optimizer.step()
