...
class joint_CE_loss(nn.Module):
    ...
    def forward(self, logits_tuple,  target):
        logits_final, logits_raw_list, feat = logits_tuple
        ...
        target_softmax_out_temp = nn.Softmax(dim=1)(logits_final)
        target_entropy_weight = Entropy(target_softmax_out_temp).detach()
        target_entropy_weight = 1 + torch.exp(-target_entropy_weight)
        target_entropy_weight = target.size(0) * target_entropy_weight / torch.sum(target_entropy_weight)
        loss_ACL = torch.mean(
            target_entropy_weight * target_softmax_out_temp.topk(2, 1)[0][:, 0] * target_softmax_out_temp.topk(2, 1)[0][
                                                                                  :,
                                                                                  1])
        return loss + loss_ACL
...
class CIFAR10_Classifier(BaseModelDNN):
    ...
    def predict(self, x):
        ...
        vvv, ccc = self.net.state_dict()['linear.weight'][4].sort()
        ccc = ccc.cpu().numpy()
        # logits_final, logits_raw = self.net(x)
        # return logits_final
        logits_final, logits_raw, feat = self.net(x)
        return logits_final, feat, ccc
    ...
    def predict2(self, x):
        ...
        target_entropy_weight = Entropy(target_softmax_out_temp).detach()
        target_entropy_weight = 1 + torch.exp(-target_entropy_weight)
        target_entropy_weight = logits_final.size(0) * target_entropy_weight / torch.sum(target_entropy_weight)
        loss_ACL = torch.mean(
            target_entropy_weight * target_softmax_out_temp.topk(2, 1)[0][:, 0] * target_softmax_out_temp.topk(2, 1)[0][
                                                                                  :,
                                                                                  1])

        return logits_final# + logits_raw[0] + logits_raw[1] + loss_ACL
        # return logits_raw[0]
    def predict_AA(self, x):
        assert not self.net.training
        # logits_final, logits_raw = self.net(x)
        logits_final, logits_raw, feat = self.net(x)
        target_softmax_out_temp = nn.Softmax(dim=1)(logits_final)
        ...
        # return  logits_final# + loss_ACL#logits_raw[1]# + logits_raw[1]#logits_final + loss_ACL +
        # return logits_raw[0]
        return logits_final, feat
    
    def predict_atk(self, x):
        assert not self.net.training
        # logits_final, logits_raw = self.net(x)
        logits_final, logits_raw, feat = self.net(x)
        return logits_final, logits_raw, feat
    ...
    if not args.is_Train:
        ...
        if True:
            ...
            for attack_class, attack_kwargs in lst_attack:
                ...
                message = topk_defense_success_rate(model.predict_AA, model.predict, model.predict_atk, model.predict2, test_loader, attack_class, attack_kwargs, device="cuda", topk=1)[-1]
                logger.info(message)
        else: 
